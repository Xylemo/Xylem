...
UserInputService.InputBegan:Connect(function(input, gp)
	...
	if input.KeyCode == Enum.KeyCode.H then
		...
		if not vehicle then
			warn("Vehicle GUID not found.")
			return
		end
		
		local ok, result = pcall(function()
			return CallRemote(rfGet, "toggle_equip_item", vehicle)
		end)

		if not ok then
			warn("Toggle equip failed:", result)
		else
			print("Toggle equip success. Server returned:", result)
		end
	end
end)
